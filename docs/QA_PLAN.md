...
- 仅上游超时和 429 按策略重试，网络错误与认证错误不自动重试；
- 批量任务支持单项失败并返回逐项结果；
...
- 参考图 Data URI 可能造成内存峰值，必须设置输入大小上限；第一版固定请求 URL 输出并自动下载。
