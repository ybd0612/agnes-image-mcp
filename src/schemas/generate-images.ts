...
const item = z.object({ id: z.string().min(1).max(100), prompt: z.string().min(1).max(10000), model: z.string().min(1).optional(), size: z.enum(sizeValues), ratio: z.enum(ratioValues).optional(), images: z.array(z.string().min(1)).max(8).optional(), output: z.enum(['url','base64']).default('url') }).strict();
