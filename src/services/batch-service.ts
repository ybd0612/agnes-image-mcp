...
export async function generateBatch(service: ImageService, input: GenerateImagesInput) {
  ...
  for (let index = 0; index < input.items.length; index++) {
    ...
    try { const image = await service.generate(item); results.push({ index, ...(item.id === undefined ? {} : { id: item.id }), success: true, image }); }
    catch (e: any) {
      results.push({ index, ...(item.id === undefined ? {} : { id: item.id }), success: false, error: { code: e.code || 'UPSTREAM_BAD_RESPONSE', message: e.message || '生成失败' } });
